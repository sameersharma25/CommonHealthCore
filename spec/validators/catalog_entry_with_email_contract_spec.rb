...
RSpec.describe CatalogManagement::CatalogEntryWithEmailContract, dbclean: :after_each do
  ...
  describe "with catalog entry params" do
    ...
    context 'for failure case' do
      before do
        @result = subject.call(params.tap{|p| p.delete(:email)})
      end
      ...
      it 'should return any errors' do
        expect(subject.call(params.tap{|p| p[:email]='test.com'}).errors.to_h).to eq({:email=>["must be a valid email", "user not found"]})
      end
    end
  end
end